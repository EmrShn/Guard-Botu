const Discord = require('discord.js'); // EmrSahin Guard
...
const queue = new Map();
const ffmpeg = require("ffmpeg"); // EmrSahin Guard
...
client.commands = new Discord.Collection(); // EmrSahin Guard
...
client.reload = command => {
    return new Promise((resolve, reject) => { // EmrSahin Guard
        try {
...
client.elevation = message => {
    ...
    let permlvl = 0;
    if (message.member.hasPermission("BAN_MEMBERS")) permlvl = 2; // EmrSahin Guard
...
client.login(ayarlar.token); // EmrSahin Guard



//-------------KOMUTLAR-------\\

client.on("channelDelete", async function(channel) { // EmrSahin Guard
...
let channelp = channel.parentID;
  ...
  channel.clone().then(z => { // EmrSahin Guard
...
client.on("message", async message => {
  ...
  const lus = await db.fetch(`reklam_${message.guild.id}`) // EmrSahin Guard
  if (lus) {
...
client.on("messageUpdate", async message => { // EmrSahin Guard
...
//kick
client.on("message", async message => { // EmrSahin Guard
...
if (!lus) return; // EmrSahin Guard
});



///spamke
const antispam = require("discord-anti-spam-tr"); // EmrSahin Guard
antispam(client, {
...
client.on("messageUpdate", async message => {
  ...
  if (lus) {
    ...
    if (reklamengel.some(word => message.content.toLowerCase().includes(word))) {
      try {
        if (!message.member.permissions.has('KICK_MEMBERS')) { // EmrSahin Guard
          message.delete();
...
//modlog 
client.on("messageDelete", async message => { // EmrSahin Guard
...
new Discord.MessageEmbed()
          .setDescription(`Aleyküm Selam Hoşgeldin!`) // EmrSahin Guard
)
      }
  }
...
/////Rol Koruma
client.on("roleDelete", async role => { // EmrSahin Guard
  ...
  role.guild.roles.create({ data: {
          ...
          mentionable: role.mentionable, // EmrSahin Guard
